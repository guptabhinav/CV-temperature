...
print *, 'NOTE: this program skips the first few column of the vfict file'
read(1,*)
read(1,*)
read(1,*)
...
!  temp=temp+mass(i)*vfict(i)*vfict(i)/gasconst*convert_to_K
  ...
  temp=temp+mass(i)*vfict(i)*vfict(i)
  end do
  temp = temp/dfloat(ncv)              ! averaging over all CVs
  temp = temp/gasconst*convert_to_K
